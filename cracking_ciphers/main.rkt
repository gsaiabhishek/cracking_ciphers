...
(define (cc1 l1 l2)
  (if (null? l1) l2
        (cc1 (cdr l1) (append l2 (converttokeys '() ((car l1) '()))))))
(define (converttokeys l3 l)
  (if (null? l) l3 (converttokeys (append l3 (let ([dict (algo:dictionary-closure (utils:add-substitution (car l) key))])
                                                    (if (equal? #f dict) '() (let ([swe (algo:secret-word-enumeration  dict)])
                                                                               (if (equal? #f swe) '() (begin (map (lambda (z) (utils:show-key z)) swe) swe)))))) (cdr l))))
